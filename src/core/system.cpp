...
void System::UpdateRunningGame(const char* path, CDImage* image, bool force)
{
  ...
  s_cheat_list.reset();
  if (g_settings.auto_load_cheats && !Achievements::IsChallengeModeActive())
...
{
  if (!IsValid() || Achievements::IsChallengeModeActive())
    return false;
...
{
  if (System::GetRunningCode().empty() || Achievements::IsChallengeModeActive())
    return false;
