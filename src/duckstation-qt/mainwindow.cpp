...
void MainWindow::onSystemStarting()
{
  ...
  updateEmulationActions(true, false, Achievements::IsChallengeModeActive());
}
...
void MainWindow::onSystemStarted()
{
  ...
  s_system_valid = true;
  updateEmulationActions(false, true, Achievements::IsChallengeModeActive());
  updateWindowTitle();
...
void MainWindow::onSystemDestroyed()
{
  s_system_valid = false;
  s_system_paused = false;
  updateEmulationActions(false, false, Achievements::IsChallengeModeActive());
  switchToGameListView();
...
void MainWindow::populateGameListContextMenu(const GameList::Entry* entry, QWidget* parent_window, QMenu* menu)
{
  ...
  if (!entry->serial.empty())
  {
    ...
    const bool challenge_mode = Achievements::IsChallengeModeActive();
...
void MainWindow::onGameListEntryContextMenuRequested(const QPoint& point)
{
  ...
  if (entry)
  {
    ...
    if (!s_system_valid)
    {
      ...
      if (m_ui.menuDebug->menuAction()->isVisible() && !Achievements::IsChallengeModeActive())
      {
