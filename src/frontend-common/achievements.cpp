...
bool Achievements::IsChallengeModeEnabled()
{
  return g_settings.achievements_challenge_mode;
}

bool Achievements::IsChallengeModeActive()
{
  return s_active && s_challenge_mode;
}
...
void Achievements::UpdateSettings(const Settings& old_config)
{
  ...
  if (g_settings.achievements_test_mode != old_config.achievements_test_mode ||
      ...
      g_settings.achievements_challenge_mode != old_config.achievements_challenge_mode)
  {
    Shutdown();
    Initialize();
    return;
...
void Achievements::GetUserUnlocks()
{
  ...
  request.hardcore = static_cast<int>(s_challenge_mode);
...
void Achievements::RAIntegration::InitializeRAIntegration(void* main_window_handle)
{
  ...
  s_challenge_mode = RA_HardcoreModeIsActive() != 0;
