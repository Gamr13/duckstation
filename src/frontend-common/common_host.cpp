...
void CommonHost::RenderDebugWindows()
{
...
static void HotkeyLoadStateSlot(bool global, s32 slot)
{
  ...
  {
    Achievements::DisplayBlockedByChallengeModeMessage();
    return;
  }
...
DEFINE_HOTKEY("Screenshot", TRANSLATABLE("Hotkeys", "General"), TRANSLATABLE("Hotkeys", "Save Screenshot"),
  ...
  DEFINE_HOTKEY("OpenAchievements", TRANSLATABLE("Hotkeys", "General"),
    TRANSLATABLE("Hotkeys", "Open Achievement List"), [](s32 pressed) {
    if (pressed && System::IsValid())
    {
      if (!m_fullscreen_ui_enabled || !FullscreenUI::OpenAchievementsWindow())
      {
        AddOSDMessage(
          TranslateStdString("OSDMessage", "Achievements are disabled or unavailable for  game."),
          10.0f);
      }
    }
  });

  DEFINE_HOTKEY("OpenLeaderboards", TRANSLATABLE("Hotkeys", "General"),
    TRANSLATABLE("Hotkeys", "Open Leaderboard List"), [](s32 pressed) {
    if (pressed && System::IsValid())
    {
      if (!m_fullscreen_ui_enabled || !FullscreenUI::OpenLeaderboardsWindow())
      {
        AddOSDMessage(
          TranslateStdString("OSDMessage", "Leaderboards are disabled or unavailable for  game."),
          10.0f);
      }
    }
  });
...
DEFINE_HOTKEY("LoadSelectedSaveState", TRANSLATABLE("Hotkeys", "Save States"),
              TRANSLATABLE("Hotkeys", "Load From Selected Slot"), [](s32 pressed) {
                if (!pressed)
                {
                  if (!Cheevos::IsChallengeModeEnabled())
...
#define MAKE_LOAD_STATE_HOTKEY(global, slot, name)                                                                     \
  DEFINE_HOTKEY(global ? "LoadGameState" #slot : "LoadGlobalState" #slot, TRANSLATABLE("Hotkeys", "Save States"),      \
                name, [](s32 pressed) {                                                                                \
                  if (!pressed)                                                                                        \
                  {                                                                                                    \
                    HotkeyLoadStateSlot(global, slot);                                                                 \
                  }                                                                                                    \
                })
#define MAKE_SAVE_STATE_HOTKEY(global, slot, name)                                                                     \
  DEFINE_HOTKEY(global ? "SaveGameState" #slot : "SaveGlobalState" #slot, TRANSLATABLE("Hotkeys", "Save States"),      \
                name, [](s32 pressed) {                                                                                \
                  if (!pressed)                                                                                        \
                  {                                                                                                    \
                    HotkeySaveStateSlot(global, slot);                                                                 \
                  }                                                                                                    \
                })
