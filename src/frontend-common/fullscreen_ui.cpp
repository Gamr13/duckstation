...
void FullscreenUI::DoStartPath(std::string path, std::string state, std::optional<bool> fast_boot)
{
  ...
  // if (System::IsExeFileName(path.c_str()) || System::IsPsfFileName(path.c_str()) || Cheevos::IsChallengeModeActive())
  // state = {};
...
void FullscreenUI::DrawLandingWindow()
{
  ...
  if (BeginFullscreenColumnWindow(570.0f, LAYOUT_SCREEN_WIDTH, "menu", UIBackgroundColor))
  {
    ...
    if (MenuButton(" " ICON_FA_PLAY_CIRCLE "  Resume",
                   ...
                   !Achievements::IsChallengeModeActive()))
    {
...
void FullscreenUI::DrawControllerSettingsPage()
{
  ...
  static constexpr const std::array<u32, Controller::NUM_CONTROLLER_PORTS> mtap_port_order = {{0, 2, 3, 4, 1, 5, 6, 7}};
  ...
  for (u32 global_slot : mtap_port_order)
  {
    ...
    for (u32 macro_index = 0; macro_index < Controller::NUM_MACRO_BUTTONS_PER_CONTROLLER; macro_index++)
    {
...
void FullscreenUI::PopulateSaveStateScreenshot(SaveStateListEntry* li, const ExtendedSaveStateInfo* ssi)
{
  ...
  {
    li->preview_texture = g_host_display->CreateTexture(PLACEHOLDER_ICON_WIDTH, PLACEHOLDER_ICON_HEIGHT, 1, 1,
...
void DrawAchievementWindow()
{
        ...
        ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse))
  {
    ...
    if (visible)
    {
      ...
      ImGui::PushFont(g_large_font);
      ImGui::RenderTextClipped(title_bb.Min, title_bb.Max, text.GetCharArray(),
                                text.GetCharArray() + text.GetLength(), nullptr, ImVec2(0.0f, 0.0f), &title_bb);
      ImGui::PopFont();
...
static void DrawLeaderboardListEntry(const Cheevos::Leaderboard& lboard)
{
  ...
  ImGui::PushFont(g_large_font);
  ImGui::RenderTextClipped(title_bb.Min, title_bb.Max, lboard.title.c_str(),
                            lboard.title.c_str() + lboard.title.size(), nullptr, ImVec2(0.0f, 0.0f), &title_bb);
  ImGui::PopFont();
...
void DrawLeaderboardsWindow()
{
        ...
        ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse))
  {
    ImRect bb;
    bool visible, hovered;
    bool pressed = MenuButtonFrame("leaderboards_heading", false, heading_height_unscaled, &visible, &hovered,
                                    &bb.Min, &bb.Max, 0, alpha);
    UNREFERENCED_VARIABLE(pressed);
